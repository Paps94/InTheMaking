    ...
    <div class="banner-table">
        ...
        <div class="container">
          <div class="grid isotope row row-45 row-md-60 clearleft-custom">
            @foreach ($tmissues as $tmissue)
            <div class="isotope-item col-lg-4 col-md-6 col-sm-12 col-xs-12 wow fadeInUp element-item {{$tmissue->priority}}" data-wow-delay=".2s">
              <div class="card-group">
                <div class="card">
                  <div class="thumbnail"><a class="img-link" href="/tmissues/{{$tmissue->id}}"><img src="{{ asset('images/home/contractthumb.jpg') }}"/><span class="thumbnail-price prio" id="{{$tmissue->priority}}">{{$tmissue->priority}}<span class="mon"> Priority</span></span></a></div>
                    <div class="card-body">
                      <div class="caption">
                        <h4 class="name">{{$tmissue->name}}</h4>
                        <ul class="describe-1">
                          <label>Start Date - End Date:</label>
                          <li><img src="{{ asset('images/fonts/startDate.svg') }}" class="svgIMGcard"/>{{Carbon\Carbon::parse($tmissue->created_at)->format('l jS \\of F Y')}}</li>
                          <li class="dateIssued" style="display: none;"><img src="{{ asset('images/fonts/startDate.svg') }}" class="svgIMGcard"/>{{$tmissue->created_at}}</li>

                          <li><img src="{{ asset('images/fonts/endDate.svg') }}" class="svgIMGcard"/>{{Carbon\Carbon::parse($tmissue->deadline)->format('l jS \\of F Y')}}</li>
                          <li class="dateTerminated" style="display: none;"><img src="{{ asset('images/fonts/endDate.svg') }}" class="svgIMGcard"/>{{$tmissue->deadline}}</li>

                          <label class="labelStyle">Deadline:</label>
                          <li class="deadline"><img src="{{ asset('images/fonts/deadline.svg') }}" class="svgIMGcard"/>{{Carbon\Carbon::now()->parse($tmissue->deadline)->diffForHumans()}}</li>
                        ...
                        <form action="{{ route('tmissues.destroy', [$tmissue->id]) }}" method="POST" >
                          ...
                          {{ csrf_field() }}
                          <button title="Delete" type="submit" href="{{ route('tmissues.destroy', [$tmissue->id]) }}" class="btn btn-success btn-success-transparent btn-block deleteBtn"> Completed </button>
                        </form>
                      </div>
                    </div>
                  </div>
                ...
                <div class="card-footer">
                  <small class="text-muted">Last updated: {{$tmissue->updated_at->diffForHumans()}}</small>
                </div>
              </div>
            @endforeach
